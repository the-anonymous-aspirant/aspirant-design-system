...
Props: `options` (`[{ value, label, disabled?, icon?, controls?, attrs? }]`), `modelValue` (v-model — the selected `value`), `size` (`sm` | `md`), `as` (`radiogroup` default | `tabs`), `ariaLabel`, `disabled` (whole group). Emits `update:modelValue`.

Two per-option seams (§3.94, #4562), both additive: **`attrs`** is an object of DOM attributes spread onto the member `<button>` — a `data-test` hook, an `id`, a `title`, `aria-describedby`. It carries no paint opinion and is bound first, so the DS-owned attributes (`type`, `role`, `aria-checked`/`aria-selected`/`aria-controls`, `tabindex`, `disabled`, the click/keydown handlers) are reserved and win over a colliding key; `class` merges additively (a layout/test hook, not a seam to restyle selection). **`icon`** is a closed typed descriptor `string | { src, fallback? }`: a glyph string renders as text (unchanged); the object form renders a *decorative* `<img alt="" aria-hidden="true">` in the same icon position, sized in `em` so it tracks the member's font size, showing `fallback` until the image loads and again if it errors (§1.4). In both forms `label` is the member's sole accessible name. There is deliberately **no** per-option content slot (§3.94 rejects it, as §3.88 rejected `rowClass`).
